...
function reward_function(rlctrnn, windows, learning)
    ...
    update_windows(rlctrnn, windows)
    windows.index +=1
    if !learning
        return 0
    end
    # rlctrnn.reward =  mean(windows.window_b)/0.01-mean(windows.window_a)/0.01 # double window
    rlctrnn.reward = rlctrnn.performance - mean(windows.window_a) # single window
   return rlctrnn.reward#/windows.dt
end
function update_windows(rlctrnn, windows)
    #rlctrnn.performance = (sum(abs.(rlctrnn.outputs .- rlctrnn.prev_outputs)))/windows.dt
    # rlctrnn.performance = (sum(abs.(rlctrnn.outputs .- rlctrnn.prev_outputs)))/windows.dt
    # windows.window_a[windows.window_index] = rlctrnn.performance
    #rlctrnn.prev_outputs .= rlctrnn.outputs
    rlctrnn.performance = sum(abs.(rlctrnn.outputs .- rlctrnn.prev_outputs))/rlctrnn.n
    # windows.window_b[windows.window_index] = windows.distance[windows.delayed_index] # double window
    # windows.window_a[windows.window_index] = windows.window_b[mod1(windows.window_index+5, length(windows.window_b))] # double window
    windows.window_a[windows.window_index] = rlctrnn.performance # single window
    rlctrnn.prev_outputs .= rlctrnn.outputs
end
...
# original = recoverParameters(system[1])
