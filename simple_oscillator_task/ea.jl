...
struct MicrobParamsIsland
    ...
    genesize::Int64 #number of genes
    N::Int64 #number of neurons
    islandN::Int64 #number of microbes in a deme
    mutateProb::Float64 #mutation probability
    recombProb::Float64
    generations::Int64 #number of generations
    generator::String
...
function createMicrobial(popsize::Int64,N::Int64, demesize::Int64,
    ...
    genesize = N*N+2*N
    params = MicrobeParams(popsize, genesize, N, demesize-1, mutateProb,
...
function mutate(microbial, loser)
    params = microbial[1]
    pop = microbial[2]
    if any(pop.pop[loser,:] .> 1) || any(pop.pop[loser,:] .< -1)
        throw("Out of bounds before mutation")
    end
    ...
    # check if pop.pop[loser,:] is in bounds
    if any(pop.pop[loser,:] .> 1) || any(pop.pop[loser,:] .< -1)
        throw("Out of bounds after clamp")
    end
end
...
# generate random 1 or negative 1
